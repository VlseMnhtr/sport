class SportCommentItem { 
    ...
    Render() {
        ...
        return `
          <article width="200px" id="sportcomment_${this.id}"  data-sport-id="${this.id}">
            <img height="130px" width="300px" style="" src="${this.thumb}" class="sportCom-thumb"/>
            <div class="sportCom-text">
              <a href="urlpage.html?id=${this.id}" class="sportCom-link">
                <p class="sportCom-title" contenteditable="true" id="recentnews_title_${this.id}">
                  <span>${this.title}</span>: ${this.name}
                </p>
              </a>
...
document.querySelectorAll('.sportCom-link').forEach(link => {
    link.addEventListener('click', (event) => {
      event.preventDefault();
      const sportId = link.closest('article').getAttribute('data-sport-id');
      const sportSection = document.getElementById(`sportcomment_${sportId}`);
      // show/hide the sport section using DOM manipulation
    });
  });
